...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            ...
            MapplockTheme(darkTheme = uiState.isDarkMode) {
                        ...
                        if (androidx.core.content.ContextCompat.checkSelfPermission(context, Manifest.permission.ACCESS_FINE_LOCATION) == PackageManager.PERMISSION_GRANTED) {
                            ...
                            fusedLocationClient.getCurrentLocation(Priority.PRIORITY_HIGH_ACCURACY, null)
                                .addOnSuccessListener { loc ->
                                    loc?.let { viewModel.onLocationChanged(it.latitude, it.longitude) }
                                }
                        }
                    } catch (_: Exception) {
                ...
                Surface(
                    ...
                    AnimatedContent(
                        targetState = when {
                            showSplash -> "intro_splash"
                            uiState.isFirstRun && !uiState.isLanguageSelected -> "language_selection"
                            uiState.isFirstRun && !uiState.hasLocationPermission -> "location_onboarding"
                            uiState.isFirstRun && !uiState.introGuideCompleted -> "intro_guide"
                            uiState.isFirstRun && !uiState.disclaimerAccepted -> "disclaimer"
                        ...
                        when (target) {
                            ...
                            "intro_guide" -> {
                                IntroGuideScreen(
                                    onFinished = { viewModel.completeIntroGuide() }
                                )
                            }
                            "disclaimer" -> {
                                DisclaimerScreen(
                                    onAccepted = { viewModel.completeDisclaimer() }
                                )
                            }
                            "downloading" -> {
                            ...
                            "vault" -> {
                                VaultScreen(
                                    ...
                                    onAddFilesToFolder = { uris, folder -> viewModel.addFilesToVault(uris, com.aitoyz.mapplock.model.FileCategory.OTHER, folder) },
    ...
    override fun onPause() {
        ...
        if (!isChangingConfigurations && !viewModel.isPerformingAction()) {
            ...
            val isStillMe = tasks.isNotEmpty() && tasks[0].topActivity?.packageName == packageName
